...
\documentclass[bsc,twoside,singlespacing,parskip,deptreport,logo,notimes,normalheadings]{infthesis}
% for BSc, BEng etc.
...
\abstract{ \setcounter{page}{1} Data-flow analysis is one of the
  ...
  The software was wildly successful in increasing the learning
  capacity of the evaluation subjects. The results collected present
  strong evidence of the need for more interactive learning tools,
  particularly in engineering subjects. Responses to the feedback
  survey were overwhelmingly positive, achievement of learning
  outcomes was well above average, users were kept engaged by the
  platform, and all but one participant stated that they would use the
  system alongside other methods of study.}
...
\vspace{-2mm}
    ...
    \begin{textblock}{50}(40,214)
      \centering
      \begin{tikzpicture}[y=-1cm, remember picture]
        % vertices
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n0) at (0 , 0) {Input (Source)};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n1) at (4 , 0) {Lexing};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n2) at (8 , 0) {Syntactic Analysis};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n3) at (12 , 0) {Semantic Analysis};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n4) at (12 , 2) {Optimisation};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n5) at (8 , 2) {Code Generation};
        \node[block, below right, minimum height=1.25cm, text width=2.25cm] (n6) at (0 , 2) {Output (Target)};

        \draw[-{>[scale=1.5]}] (n0) -- (n1);
        \draw[-{>[scale=1.5]}] (n1) -- (n2);
        \draw[-{>[scale=1.5]}] (n2) -- (n3);
        \draw[-{>[scale=1.5]}] (n3) -- (n4);
        \draw[-{>[scale=1.5]}] (n4) -- (n5);
        \draw[-{>[scale=1.5]}] (n5) -- (n6);
      \end{tikzpicture}
    \end{textblock}
    ...
    \begin{minipage}{\dimexpr\textwidth-6cm}
      ...
      The \gls{ir} may be used to form other representations; for
      example a \gls{cfg} models the possible execution paths for a
      given program. Each node in the graph represents an instruction
      or block of instructions, each edge represents an execution path
      leading from one instruction to another. A node can have
      multiple outward edges if it is a branching
      instruction, and branches may point backward in the control-flow. An
      example of a simple \gls{controlflowgraph} can be seen in
      fig. \ref{fig:cfgexample}.
      ...
      \textcolor{magenta}{magenta} on the right).

    \end{minipage}

    \begin{textblock}{50}(115,33)
      \centering
      \begin{tikzpicture}[y=-1cm, remember picture]
        % vertices
        \node[block, text width=2cm] (n1) at (2 , 0) {\tt a = X[i]};
        \node[block, text width=2cm] (n2) at (2 , 1) {\tt b = Y[i]};
        \node[block, text width=2cm] (n3) at (2 , 2) {\tt if a > b};
        \node[block, text width=2cm] (n4) at (0.75 , 3) {\tt Z[i] = a};
        \node[block, text width=2cm] (n5) at (3.25 , 3) {\tt Z[i] = b};
        \node[block, text width=2cm] (n6) at (2 , 4) {\tt i++};
        ...
        % edges
        \draw[edge] ($ (n1.north) + (0,-0.7) $) to (n1.north);
        \draw[edge] (n1.south) to (n2.north);
        \draw[edge] (n2.south) to (n3.north);
        \draw[edge] (n3.south) to (n4.north);
        \draw[edge] (n3.south) to (n5.north);
        \draw[edge] (n4.south) to (n6.north);
        \draw[edge] (n5.south) to (n6.north);
        \draw[edge] (n6.south) to (n7.north);
        \draw[edge] (n7.south) to (n8.north);
        \draw[edge] (n7.south) to [out=-30, in=270] (4.8, 5) -- (4.8, 0.4) to [out=90, in=30] (n1.north);
    ...
    \section{Data-Flow Analysis}        
    During construction, the \gls{ir} is often annotated to assist in
    optimisation and code generation. Data-flow analysis is one of the
    techniques used to perform this annotation. In general, it is
    performed in the semantic analysis or optimisation stage and is
    used to compute information about data flowing {\em in} and {\em
      out} of each node in the program's \gls{cfg}. Many types of
    analysis can be performed and the information gathered is used to
    inform the decisions of optimising compilers. A brief list of
    analyses and their purposes can be found in appendix
    \ref{appx:analysistypes}.
	...
	Sets of values in a \gls{dataflow} problem have a partial
        ...
        Partial orders can be visualised using a
        \gls{hassediagram}. Each node represents a value in $L$,
        whilst each edge represents an order between nodes: an edge
        from $a$ to $b$ indicates that $a \ge b $. To calculate the
        meet of two sets $a$ and $b$, we simply find the greatest
        (with respect to the order) common descendent of $a$ and $b$
        in the diagram. 

        The \gls{meetsemilattice} for a \gls{dataflow} can be
        expressed using a {\em \gls{hassediagram}}, shown in
        fig. \ref{fig:meethasse}.  In order to reduce clutter, a
        transitive reduction has been performed on the graph; for
        instance, the edge between {\tt \{a\}} and {\tt \{a,b,c\}} is
        represented by the path
        ${\tt \{a\}} \rightarrow {\tt\{a,b\}} \rightarrow {\tt
          \{a,b,c\}}$.        
...
conceptual problems.
    ...
    \begin{figure}[!ht]
      ...
      \captionsetup{justification=centering}
      \caption{An overview of the proposed system architecture.}\label{fig:sysarch}
    \end{figure}
...
\ref{chap:impl-frontend}.
    ...
    \begin{wrapfigure}[15]{r}{6cm}
      ...
      \captionsetup{width=6cm,justification=centering}
      \captionof{figure}{A control-flow graph.}
      \label{fig:impl-included-cfg}
    \end{wrapfigure}

    The three \gls{dataflow} frameworks operate on programs written in
    \gls{iloc} (see \S\ref{sec:impl-iloc}). ILOC programs are able to
    write to three types of storage: registers, main memory and the
    comparison register. To allow users to translate examples in other
    languages or pseudocode into ILOC and obtain the same solution,
    these \gls{dataflow}s have been defined to operate solely on
    registers.
